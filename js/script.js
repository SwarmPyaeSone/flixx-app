const global = {
    ...
    api: {
        apiKey: '4350dc2f509600b22127fd3ec0b86dc7',
...
async function displayPopularMovies() {
    ...
    results.forEach(movie => displayMovies(movie))
}
...
async function displayPopularTvShows() {
    ...
    results.forEach(show => displayShows(show))
}
...
function displayShows(show) {
    ...
    const releaseDate = document.createElement('small');
    releaseDate.classList.add('text-muted');
    releaseDate.textContent = `Release: ${show.first_air_date}`;
...
async function displayMovieDetails() {
    const movieId = window.location.search.split('d')[1];
...
async function displayShowDetails() {
    const showId = window.location.search.split('d')[1];
...
async function search() {
    ...
    if(global.search.term !== '' && global.search.term !== null) {
        ...
        showAlert('Please type in the name.', 'alert-error');
    }
}
...
function displaySearchResults(results) {
    ...
    results.forEach(result => displaySearchResultItems(result));
    displayPagination();
}

function displaySearchResultItems(result) {
    const divCard = document.createElement('div');
    divCard.classList.add('card');
    ...
    const cardTitle = document.createElement('h5');
    cardTitle.classList.add('card-title');
    cardTitle.textContent = `${global.search.type === 'movie' ? result.title : result.name}`;

    const cardText = document.createElement('p');
    cardText.classList.add('card-text');
    const releaseDate = document.createElement('small');
    releaseDate.classList.add('text-muted');
    releaseDate.textContent = `Release: ${global.search.type === 'movie' ? result.release_date : result.first_air_date}`;
    ...
    document.getElementById('search-results-heading').innerHTML = `
        <h2>${global.search.results.length} of ${global.search.totalResults} Results for ${global.search.term}</h2>
    `;
